...
import { MarkdownContent, Slide } from './types.js';
...
export class MarkdownParser {
  ...
  private parseSlide(section: string): Slide | null {
    const lines = section.split('\n').filter(line => line.trim());
    if (lines.length === 0) return null;

    let title = '';
    const content: string[] = [];
    let slideType: 'title' | 'content' | 'image' | 'quote' = 'content';
  ...
  extractImages(content: string): Array<{url: string, alt: string}> {
